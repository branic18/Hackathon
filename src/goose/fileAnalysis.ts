 ...
 */
export async function findFilesUsingPackage(
  packageName: string, 
  projectRoot: string
): Promise<string[]> {
    ...
    for (const pattern of patterns) {
      try {
        const { stdout } = await execAsync(
          `rg -l --type js --type ts --type jsx --type tsx --type json --type yaml --type yml "${pattern}" "${projectRoot}"`,
          { timeout: 5000 }
        );
        
        if (stdout.trim()) {
          const files = stdout.trim().split('\n')
            .filter(file => isPathWithinRoot(file, projectRoot))
...
async function fallbackFileSearch(
  ...
  if (!isPathWithinRoot(packageJsonPath, projectRoot)) {
 ...
 */
function sanitizeCodeSnippet(code: string): string {
  ...
  try {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      if (packageJson.dependencies?.[packageName]) {
        ...
        const hasBuildToolInPath = paths.some(path => 
          path.some(pkg => buildToolPatterns.some(tool => pkg.includes(tool)))
        );
