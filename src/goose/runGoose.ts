...
import { GooseVulnInsight, VulnContext } from "./types";
import { 
  sanitizeWorkingDirectory, 
  getMinimalEnvironment 
} from "./security";
import { createGooseValidator } from "./validator";

/**
 * Secure Goose execution with comprehensive input sanitization and output validation
 */
export async function runGooseForVuln(
  context: VulnContext
  ...
  return new Promise((resolve, reject) => {
    void (async () => {
      ...
      proc.on("close", (code) => {
        if (code !== 0) {
          return reject(
            new Error(`Goose exited with code ${code}: ${stderr.substring(0, 500)}`)
          );
        }
 ...
 */
export async function runGooseWithRetry(
  context: VulnContext,
  maxRetries: number = 2
): Promise<GooseVulnInsight> {
  let lastError: Error;
 ...
 */
export { runGooseForVuln as runGooseForVulnLegacy };
